$(document).ready(function () {
    ...
    if(tabs){
        const tabsBtn = tabs.querySelectorAll('.tabs-menu li');
        const tabsContent = tabs.querySelectorAll('.tabs-content__item');
        for(let i = 0; i < tabsBtn.length; i++){
            tabsBtn[i].addEventListener('click', function(){
                ...
                const thisdata = this.getAttribute('data-button');
                for(let k = 0; k < tabsContent.length; k++){
                    tabsContent[k].classList.remove('active');
                    const contentData = tabsContent[k].getAttribute('data-content');
